...
if(mysqli_num_rows($conn->query("SELECT * FROM `reservations` WHERE ((`check_in` between '".$_SESSION["reservation"][0]."' AND '".$_SESSION["reservation"][1]."') OR (`check_out` between '".$_SESSION["reservation"][0]."' AND '".$_SESSION["reservation"][1]."')) AND `room_id` = '".$_SESSION["reservation"][5]->id."' AND `cancelled_by` IS NULL")) > 0) {
    ...
    $id = $conn->insert_id;
    $stmt = $conn->query("INSERT INTO `reservation_info`(`reservation_id`,`first_name`,`last_name`,`email`,`phone_number`,`num_adult`,`num_child`,`package_id`) VALUES('$id', '" . $_SESSION["reservation"][6] . "','" . $_SESSION["reservation"][7] . "','" . $_SESSION["reservation"][8] . "','" . $_SESSION["reservation"][9] . "','" . $_SESSION["reservation"][2] . "','" . $_SESSION["reservation"][3] . "','" . $_SESSION["reservation"][11]->id . "')");
